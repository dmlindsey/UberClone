...
//  UberClone
//
//  Created by Dean Lindsey on 05/03/2018.
                ...
                if let rideRequestDict = snapshot.value as? [String:AnyObject] {
                    if let driverLat = rideRequestDict["driverLat"] as? Double {
                        if let driverLon = rideRequestDict["driverLon"] as? Double {
                            ...
                            if let email = FIRAuth.auth()?.currentUser?.email {
                                FIRDatabase.database().reference().child("RideRequests").queryOrdered(byChild: "email").queryEqual(toValue: email).observe(.childChanged, with: { (snapshot) in
